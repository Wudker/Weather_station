...
static const int TIME_OFFSET_HOURS = 0;
...
static volatile bool mqtt_publish_pending = false;
static Dane_ESP Dane_do_mqtt;

static bool wifi_laczenie = false;
static bool wifi_bylo_polaczone = false;
static uint32_t wifi_start_ms = 0;
static uint32_t wifi_next_attempt_ms = 0;
static uint32_t wifi_retry_delay_ms = 5000;
...
static const char* NTP_SERVER_1 = "pl.pool.ntp.org";
static const char* NTP_SERVER_2 = "pool.ntp.org";
static const char* NTP_SERVER_3 = "time.google.com";
static const time_t NTP_MIN_VALID_EPOCH = 1704067200;
static const uint32_t NTP_CHECK_PERIOD_MS = 1000;
static const uint32_t NTP_LOG_GRACE_MS = 15000;

static bool ntp_uruchomiony = false;
static bool ntp_gotowy = false;
static uint32_t ntp_last_check_ms = 0;
static volatile uint32_t sd_log_pending_since_ms = 0;
static volatile uint32_t mqtt_publish_pending_since_ms = 0;
...
static void NTP_start(){
  if(ntp_uruchomiony || WiFi.status() != WL_CONNECTED) return;

  configTzTime(NTP_TZ, NTP_SERVER_1, NTP_SERVER_2, NTP_SERVER_3);
  ntp_uruchomiony = true;
  Serial.println("NTP: rozpoczecie synchronizacji");
}

static bool NTP_pobierz_czas(struct tm& czasLokalny){
  if(!ntp_uruchomiony) return false;

  time_t teraz = time(nullptr);
  if(teraz < NTP_MIN_VALID_EPOCH) return false;
  return localtime_r(&teraz, &czasLokalny) != nullptr;
}

static void NTP_wpisz_czas(Dane_ESP& d, const struct tm& czasLokalny){
  d.Day = czasLokalny.tm_mday;
  d.Month = czasLokalny.tm_mon + 1;
  d.Year = (czasLokalny.tm_year + 1900) % 100;
  d.Hour = czasLokalny.tm_hour;
  d.Minute = czasLokalny.tm_min;
}

static bool NTP_nadpisz_czas(Dane_ESP& d){
  struct tm czasLokalny;
  if(!NTP_pobierz_czas(czasLokalny)) return false;
  NTP_wpisz_czas(d, czasLokalny);
  return true;
}

static void NTP_loop(){
  if(!ntp_uruchomiony) return;
  if(millis() - ntp_last_check_ms < NTP_CHECK_PERIOD_MS) return;
  ntp_last_check_ms = millis();

  struct tm czasLokalny;
  if(!NTP_pobierz_czas(czasLokalny)) return;

  if(!ntp_gotowy){
    ntp_gotowy = true;
    Serial.printf("NTP: %04d-%02d-%02d %02d:%02d\n",
                  czasLokalny.tm_year + 1900,
    ...
    if(lastRxMs != 0){
      noInterrupts();
      NTP_wpisz_czas(Dane, czasLokalny);
      if(sd_log_pending) NTP_wpisz_czas(Dane_do_zapisu, czasLokalny);
      if(mqtt_publish_pending) NTP_wpisz_czas(Dane_do_mqtt, czasLokalny);
      interrupts();
    }
  }
}
...
static void SD_logger_loop(){
  ...
  if(!ntp_gotowy &&
     (WiFi.status() == WL_CONNECTED || wifi_laczenie) &&
     millis() - sd_log_pending_since_ms < NTP_LOG_GRACE_MS){
    return;
  }
...
static void MQTT_publish_loop(){
  if(!mqtt_publish_pending || !mqttConnected()) return;

  if(!ntp_gotowy &&
     WiFi.status() == WL_CONNECTED &&
     millis() - mqtt_publish_pending_since_ms < NTP_LOG_GRACE_MS){
    return;
  }
  ...
  noInterrupts();
  d = Dane_do_mqtt;
  mqtt_publish_pending = false;
  interrupts();

  if(!mqttPublishMeasurement(d)){
    noInterrupts();
    if(!mqtt_publish_pending){
      Dane_do_mqtt = d;
      mqtt_publish_pending = true;
    }
    interrupts();
  }
}
...
static void Internet_check_start(){
  ...
  if((int32_t)(millis() - wifi_next_attempt_ms) < 0) return;

  Serial.println("WiFi: laczenie...");
...
static void Internet_check_loop(){
  if(WiFi.status() == WL_CONNECTED){
    if(!wifi_bylo_polaczone){
      wifi_bylo_polaczone = true;
      wifi_laczenie = false;
  ...
  if(wifi_bylo_polaczone){
    ...
    Serial.println("WiFi: utracono polaczenie");
    wifi_next_attempt_ms = millis();
  }
  ...
  if(wifi_laczenie){
    if(millis() - wifi_start_ms > WIFI_CONNECT_TIMEOUT_MS){
      ...
      Serial.print("WiFi: kolejna proba za ");
      Serial.print(wifi_retry_delay_ms / 1000);
      Serial.println(" s");

      wifi_next_attempt_ms = millis() + wifi_retry_delay_ms;
      wifi_retry_delay_ms *= 2;
      if(wifi_retry_delay_ms > WIFI_RETRY_MAX_MS){
        wifi_retry_delay_ms = WIFI_RETRY_MAX_MS;
      }
    }
    return;
  }
...
void OnDataRecv(const uint8_t *mac, const uint8_t *incomingData, int len){
  ...
  if(len == (int)sizeof(Dane_ESP)){
    ...
    Dane_do_zapisu = Dane;
    sd_log_pending = true;
    sd_log_pending_since_ms = millis();

    Dane_do_mqtt = Dane;
    mqtt_publish_pending = true;
    mqtt_publish_pending_since_ms = millis();
